...
SELECT 
    CAST(CAST(id AS NUMERIC) AS INT) AS planning_slot_id,
    name_seq,
    type_int,
    my_image,
    "Rapport" AS rapport,
    "Remarque" AS remarque,
    CAST(CAST(employee_id AS NUMERIC) AS INT) AS employee_id,
    CAST(CAST(user_id AS NUMERIC) AS INT) AS user_id,
    CAST(CAST(partner_id AS NUMERIC) AS INT) AS partner_id,
    CAST(CAST(company_id AS NUMERIC) AS INT) AS company_id,
    start_datetime::TIMESTAMP,
    end_datetime::TIMESTAMP,
    allocated_hours::NUMERIC,
    create_date::TIMESTAMP,
    ...
    state,
    motif,
    CAST(CAST(parent_id AS NUMERIC) AS INT) AS parent_id,
    ville_client,
    region_client,
    NOW() AS stg_loaded_at,
    'batch_001' AS stg_batch_id
FROM stg.planning_slot s
