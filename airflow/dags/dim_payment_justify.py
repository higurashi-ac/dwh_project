...
def build_dim_payment_justify_sql():
    """
    Build SQL dynamically for dim_payment_justify:
    ...
    cols_info = pg_hook.get_records("""
        ...
        WHERE table_schema = 'stg'
          AND table_name = 'payment_justify'
        ORDER BY ordinal_position;
    """)

    if not cols_info:
        raise ValueError("No columns found in stg.payment_justify")

    # Fixed part of dimension
    fixed_columns = [
        '"payment_justify_sk" BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY',
        '"payment_justify_id" INT UNIQUE'
    ]

    # Dynamic part, insert/upsert lists
    dynamic_columns = []
    insert_cols = ['payment_justify_id']
    insert_select = ['id AS payment_justify_id']
    update_set = []

    for col_name, data_type in cols_info:
        if col_name == "id":
            continue  # already mapped to payment_justify_id
    ...
    full_sql = create_sql + "\n" + upsert_sql
    logging.info(f"Generated SQL for dim_payment_justify:\n{full_sql}")
    return full_sql
...
with DAG(
    ...
    if load_mode.upper() == "FULL":
        truncate_task = PostgresOperator(
            task_id='truncate_dim_payment_justify',
            postgres_conn_id='postgres_public',
            sql='TRUNCATE TABLE dwh.dim_payment_justify;'
        )
        load_task = PostgresOperator(
            task_id='load_dim_payment_justify_full',
            postgres_conn_id='postgres_public',
            sql=build_dim_payment_justify_sql()
        )
        truncate_task >> load_task
    else:
        load_task = PostgresOperator(
            task_id='load_dim_payment_justify_incremental',
            postgres_conn_id='postgres_public',
            sql=build_dim_payment_justify_sql()
        )