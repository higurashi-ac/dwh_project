...
load_mode = Variable.get("load_mode_dim_sale_order_line", default_var="INCREMENTAL")

def build_dim_sale_order_line_sql():
    """
    Build SQL dynamically for dim_sale_order_line:
    ...
    if not cols_info:
        raise ValueError("No columns found in stg.sale_order_line")

    # Fixed part of dimension
    fixed_columns = [
        '"sale_order_line_sk" BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY',
        '"sale_order_line_id" INT UNIQUE'
    ]

    # Dynamic part, insert/upsert lists
    dynamic_columns = []
    insert_cols = ['sale_order_line_id']
    insert_select = ['id AS sale_order_line_id']
    update_set = []

    for col_name, data_type in cols_info:
        if col_name == "id":
            continue  # already mapped to sale_order_line_id
    ...
    full_sql = create_sql + "\n" + upsert_sql
    logging.info(f"Generated SQL for dim_sale_order_line:\n{full_sql}")
    return full_sql
...
with DAG(
    ...
    description='ETL DAG for dim_sale_order_line (auto schema + upsert)',
    ...
    if load_mode.upper() == "FULL":
        truncate_task = PostgresOperator(
            task_id='truncate_dim_sale_order_line',
            postgres_conn_id='postgres_public',
            sql='TRUNCATE TABLE dwh.dim_sale_order_line;'
        )
        load_task = PostgresOperator(
            task_id='load_dim_sale_order_line_full',
            postgres_conn_id='postgres_public',
            sql=build_dim_sale_order_line_sql()
        )
        truncate_task >> load_task
    else:
        load_task = PostgresOperator(
            task_id='load_dim_sale_order_line_incremental',
            postgres_conn_id='postgres_public',
            sql=build_dim_sale_order_line_sql()
        )